...
> 当前 `v0.1.0` 为预发布候选版：通用测试、新环境初始化、K3 实际安装，以及已安装入口的 1,200 步长蛇运行已通过；2026-09-21 补测的 43 项测试（无跳过）、终端交互及新下载模型的 1,200 步运行全部通过。本轮为 Bianbu 4.0.7 + 锁定系统库，采用电脑下载后离线导入；不是重新刷机或 K3 直连下载验收。详见 [验收状态](docs/VALIDATION.md)。
...
本次总时长 158.68 秒，每秒采样一次；游戏记录校验通过，零死亡、零保护介入，排除首步后平均推理 **109.86 ms/步**。方法、原始采样和统计边界见 [资源实测记录](docs/RESOURCES.md)。这是一次已安装入口的运行验证；后续独立环境补测已完成完整硬件回归和终端验收，详见 [验收记录](docs/VALIDATION.md)。
