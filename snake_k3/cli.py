"""Run the offline Laya Snake terminal demo on K3."""
...
def play(argv=None):
    ...
    parser.add_argument("--backend", choices=("torch", "spacemit"), default="torch")
    parser.add_argument("--full-metrics", action="store_true",
                        help="Compute the two auxiliary model estimates too (slower)")
    parser.add_argument(
        "--optimize",
        action="store_true",
        help="Unsupported on K3; this MLX-only option is rejected",
    )
    ...
    if args.optimize:
        parser.error("--optimize is MLX-only and unavailable on K3")
    engine_label = 'AI Core + CPU / FP16 模型' if args.backend == 'spacemit' else 'CPU / FP32'
    print(f"正在加载 K3 本地 Laya（{engine_label}）；不联网。", file=sys.stderr, flush=True)
    policy = LayaPolicy(
        args.model, guarded=not args.unassisted, prompt=args.prompt, optimize=args.optimize,
        threads=args.threads, full_metrics=args.full_metrics, backend=args.backend,
    )
    # CPU eager inference needs no compiled-graph warmup. The first actual
    # decision is displayed instead of spending six hidden decisions first.
    record = None
    print("开始游戏：空格暂停，R 重开，Q 退出。每一步都等待真实模型判断。", file=sys.stderr, flush=True)
    if args.record:
    ...
    stats = {
        ...
        "display_engine": policy.metadata.get("display_engine", "CPU · FP32"),
